...
class TypescriptVisitor {
            ...
            .forEach(classDeclaration => {
                ...
                classDeclaration.getProperties().forEach(property => {
                    if (!property.isPrimitive()) {
                        ...
                        // Scalars, enums, and maps use their plain name; classes use the I prefix
                        const typeDecl = property.getParent?.().getModelFile?.().getModelManager?.().getType(property.getFullyQualifiedTypeName());
                        const isTypeMap = typeDecl?.isMapDeclaration?.();
                        const isTypeEnum = property.isTypeEnum?.();
                        const isTypeScalar = property.isTypeScalar?.();
                        properties.get(typeNamespace).add((isTypeEnum || isTypeScalar || isTypeMap) ? typeName : `I${typeName}`);
                    }
                });
    ...
    visitClassDeclaration(classDeclaration, parameters) {
        ...
        const directSubclasses = classDeclaration.getDirectSubclasses() || [];
        const subclasses = directSubclasses
            .filter(subclass => subclass.getModelFile().getNamespace() === classDeclaration.getModelFile().getNamespace()) // Only consider subclasses in the same namespace for the union
            .filter(subclass => !subclass.isEnum()); // Don't include enums in the union
    ...
    visitField(field, parameters) {
        ...
        if (field.isTypeScalar?.()) {
            const typeName = ModelUtil.getShortName(field.getFullyQualifiedTypeName());
            parameters.fileWriter.writeLine(1, field.getName() + optional + ': ' + typeName + array + ';');
            return null;
        }
        ...
        if (!!parameters.flattenSubclassesToUnion && !field.isPrimitive()) {
            const typeDeclaration = field.getParent().getModelFile().getModelManager().getType(field.getFullyQualifiedTypeName());
            const directSubclasses = typeDeclaration.getDirectSubclasses() || [];
            const sameNamespaceSubclasses = directSubclasses.filter(subclass => subclass.getModelFile().getNamespace() === typeDeclaration.getModelFile().getNamespace());
            if (sameNamespaceSubclasses && sameNamespaceSubclasses.length > 1) {
    ...
    visitScalarDeclaration(scalarDeclaration, parameters) {
        ...
        parameters.fileWriter.writeLine(0, `export type ${name} = ${tsType};\n`);
        return null;
    }
    ...
    visitMapDeclaration(mapDeclaration, parameters) {
        ...
        if (ModelUtil.isPrimitiveType(mapValueType)) {
            ...
            const enumType = mapDeclaration?.getModelFile()?.getType(mapDeclaration.getValue().getType())?.isEnum?.();
            valueType = enumType ? this.toTsType(mapValueType, false, false) : this.toTsType(mapValueType, true, false);
        }
